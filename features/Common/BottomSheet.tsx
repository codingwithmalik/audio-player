...
export default function BottomSheet({
  ...
  const currentPct = useRef(COLLAPSED_PCT); // tracks live drag position, avoids reading back from GSAP
  ...
  useEffect(() => {
    ...
    if (isOpen && visible) {
      ...
      gsap
        ...
        .to(
          sheet,
          { y: `${COLLAPSED_PCT}%`, duration: 0.4, ease: "power3.out" },
          "-=0.1",
        );
  ...
  const handlePointerDown = useCallback((e: React.PointerEvent) => {
    ...
    velocity.current = 0;
    currentPct.current = expandedRef.current ? EXPANDED_PCT : COLLAPSED_PCT;
  ...
  return createPortal(
    <div className="md:hidden">
      ...
      <div
        ...
        className="fixed inset-x-0 bottom-0 z-[9999] flex bg-black/20 backdrop-blur-2xl  flex-col rounded-t-2xl border-t border-white/10 shadow-2xl"
        style={{
          height: "100dvh",
