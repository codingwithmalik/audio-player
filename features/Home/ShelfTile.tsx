...
export default function ShelfTile({ item }: { item: ShelfItem }) {
  const onPlay = item.kind === "playlist" ? item.onPlay ?? item.onClick : item.onClick;

  return (
    <button onClick={item.onClick} className="w-full text-left group hover:bg-white/10 p-2 rounded-md">
      <div className="relative w-full aspect-square mb-2">
        {item.kind === "playlist" ? (
          ...
          <SongCover src={item.coverImage} alt={item.title} fill sizes="200px" />
        )}
